...
def precipitation():
    ...
    percp_results = session.query(Measurement.date,Measurement.prcp).all()

    session.close()

    all_prcp=[]
    for date,prcp in percp_results:
        prcp_dict = {}
        prcp_dict[date] = prcp
...
def calc_temps_sd(start):
    ...
    session.close()
    tempobs={}
    tempobs["Min_Temp"]=start_results[0][0]
    tempobs["avg_Temp"]=start_results[0][1]
    tempobs["max_Temp"]=start_results[0][2]
    return jsonify(tempobs)
